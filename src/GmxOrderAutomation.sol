...
contract GmxCreateOrderAutomation {
    ...
    function checkLog(bytes calldata data)
        external
        view
        returns (bool, bytes memory)
    {
		...
		*/
        ...
        address market = MarketStore.getMarket(
            dataStore,
            key
        );
        
        address feedId = chainlinkFeedIds[market.marketToken]; // TODO: need to create this mapping
    ...
    function oracleCallback(bytes[] values, bytes extraData)
        external
        view
        returns (bool, bytes memory)
    {
    ...
    function performUpkeep(bytes calldata performData) external {
        (bytes[] chainlinkBlobs, bytes extraData) = abi.decode(
            performData,
            (bytes[], bytes)
        );
